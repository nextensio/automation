...
if __name__ == '__main__':
    ...
    ok = create_user(url, tenant, user1json)
    while not ok:
    ...
    ok = create_user(url, tenant, user2json)
    while not ok:
        print('User creation failed, retrying ...')
        time.sleep(1)
        ok = create_user(url, tenant, user2json)
    ...
    ok = create_bundle(url, tenant, bundle1json)
    while not ok:
        print('Bundle creation failed, retrying ...')
        time.sleep(1)
        ok = create_bundle(url, tenant, bundle1json)

    ok = create_bundle_attr(url, tenant, bundle1attrjson)
    while not ok:
        print('BundleAttr creation failed, retrying ...')
        time.sleep(1)
        ok = create_bundle_attr(url, tenant, bundle1attrjson)

    ok = create_bundle(url, tenant, bundle2json)
    while not ok:
        print('Bundle creation failed, retrying ...')
        time.sleep(1)
        ok = create_bundle(url, tenant, bundle2json)

    ok = create_bundle_attr(url, tenant, bundle2attrjson)
    while not ok:
        print('BundleAttr creation failed, retrying ...')
        time.sleep(1)
        ok = create_bundle_attr(url, tenant, bundle2attrjson)

    ok = create_bundle(url, tenant, bundle3json)
    while not ok:
        print('Bundle creation failed, retrying ...')
        time.sleep(1)
        ok = create_bundle(url, tenant, bundle3json)
        
    ok = create_bundle_attr(url, tenant, bundle3attrjson)
    while not ok:
        print('BundleAttr creation failed, retrying ...')
        time.sleep(1)
        ok = create_bundle_attr(url, tenant, bundle3attrjson)

    ok = create_host_attr(url, tenant, host1attrjson)
    while not ok:
    ...
    ok = create_route(url, tenant, route1json)
    while not ok:
        print('Route creation failed, retrying ...')
        time.sleep(1)
        ok = create_route(url, tenant, route1json)
        
    ok = create_route(url, tenant, route2json)
    while not ok:
        print('Route creation failed, retrying ...')
        time.sleep(1)
        ok = create_route(url, tenant, route2json)
